...
function updateGachaControls() {
    if (!isConnected) {
        ...
        gachaSection.button.textContent = "CONNECT WALLET TO START";
        gachaSection.actionSection.classList.add('hidden');
        return;
    }

    gachaSection.button.textContent = "START RANDOM";
...
function handleWalletConnected(address) {
    ...
    isConnected = true;
    walletSection.addressEl.textContent = `${address.substring(0, 6)}...${address.substring(address.length - 4)}`;
...
function init3D() { 
    ...
    const starVertices = []; 
    for (let i = 0; i < 10000; i++) { const x = THREE.MathUtils.randFloatSpread(2000); const y = THREE.MathUtils.randFloatSpread(2000); const z = THREE.MathUtils.randFloatSpread(2000); starVertices.push(x, y, z); } 
...
function animate() { 
    ...
    machine3DObjects.forEach(obj => { if (obj.nftTextMaterial) { const hue = (Date.now() * 0.0002) % 1; obj.nftTextMaterial.emissive.setHSL(hue, 1, 0.6); } }); 
...
const mobileWrapperForObserver = document.getElementById('mobile-canvas-wrapper');
const desktopWrapperForObserver = document.getElementById('desktop-canvas-wrapper');
const resizeObserver = new ResizeObserver(entries => {
    handleResize();
});
resizeObserver.observe(mobileWrapperForObserver);
resizeObserver.observe(desktopWrapperForObserver);
...
function performRoll() { 
    ...
    history.push({ date: new Date(), items: [result] }); 
    updateUI(); 
    showResultModal(result); 
}

async function triggerGachaAnimation() { 
    ...
    tl.call(() => { isShaking = false; stopSuspenseSound(); }, [], "-=0.8"); 
    tl.to(camera.position, { z: 8, duration: 0.5, ease: 'power2.out', onComplete: () => { machine3DObjects[currentMachineIndex].group.remove(capsule); performRoll(); isAnimating = false; updateGachaControls(); } }, ">0.5"); 
}
...
function createItemCard(item) { 
    ...
    if (item.rarity === 'LEGENDARY') { card.classList.add('legendary-card'); } else { card.style.boxShadow = `0 0 15px ${rarityInfo.color}60`; } 
...
function updateInventory() { 
    ...
    if (inventory.length > 0) { 
        ...
        allBtn.onclick = () => { currentInventoryFilter = 'All'; updateInventory(); }; 
        ...
        machines.forEach(machine => { 
            ...
            btn.onclick = () => { currentInventoryFilter = machine.name; updateInventory(); }; 
        ...
        document.querySelectorAll('#inventory-filter-buttons button').forEach(btn => { 
            if (btn.textContent === currentInventoryFilter) { btn.classList.add('btn-active-tab'); btn.classList.remove('bg-gray-700'); } else { btn.classList.remove('btn-active-tab'); btn.classList.add('bg-gray-700'); } 
        }); 
    } 
...
function showResultModal(result) { 
    ...
    if (hasLegendary) { 
        ...
        setTimeout(() => { legendaryFlash.classList.remove('flash'); }, 800); 
    } else { 
        playRewardSound(); 
    } 
    modal.resultDisplay.innerHTML = ''; 
    const card = createItemCard(result); 
    card.classList.add('opacity-0', 'transform', 'scale-90'); 
    modal.resultDisplay.appendChild(card); 
    setTimeout(() => { 
...
function initialize() {
    ...
    updateGachaControls(); // Add this line to set initial button state
}
